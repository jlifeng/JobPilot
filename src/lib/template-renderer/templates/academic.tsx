...
export function AcademicPreview({ resume }: TemplateProps): React.ReactElement {
    ...
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: '"Computer Modern", "CMU Serif", Georgia, "Times New Roman", serif' }}>
      <div className="mb-6">
        <div className={pi.avatar ? 'flex items-center gap-4' : 'text-center'}>
          ...
          <div>
            ...
            {pi.jobTitle && <p className="mt-0.5 text-base italic text-zinc-700">{pi.jobTitle}</p>}
      ...
      {sections.map((section) => (
        <div key={section.id} className="mb-4" data-section>
          ...
          <AcademicSectionContent section={section} lang={lang} />
        </div>
      ))}
    </div>
  );
...
function AcademicSectionContent({
  section,
  lang,
}: {
        ...
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs text-zinc-500">{dateRange(item.startDate, item.endDate, item.current, lang)}</span>
            </div>
            {item.description && (
            ...
            {item.technologies?.length > 0 && <p className="text-xs italic text-zinc-500">{lang === 'zh' ? '技术栈' : 'Technologies'}: {item.technologies.join(', ')}</p>}
        ...
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              ...
              <span className="shrink-0 text-xs text-zinc-500">{dateRange(item.startDate, item.endDate, true, lang)}</span>
            </div>
            <p className="text-sm text-zinc-600">{item.institution}{item.location ? `, ${item.location}` : ''}</p>
            {item.gpa && <p className="text-xs text-zinc-500">GPA: {item.gpa}</p>}
            {item.highlights?.length > 0 && <BulletList items={item.highlights} className="mt-0.5 list-disc pl-5" />}
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="space-y-2">
        {categories.map((category) => (
          <div key={category.id}>
            <p className="text-sm font-bold text-zinc-700">{category.name}</p>
            {category.skills?.length > 0 && (
              <ul className="mt-0.5 list-disc pl-4">
                {category.skills.map((skill, index) => <li key={index} className="text-sm text-zinc-600">{skill}</li>)}
              </ul>
            )}
          </div>
        ))}
          ...
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <div>
                ...
                {item.url && <span className="ml-1 text-xs text-zinc-500">[{item.url}]</span>}
              </div>
              {item.startDate && <span className="shrink-0 text-xs text-zinc-500">{dateRange(item.startDate, item.endDate, true, lang)}</span>}
            </div>
  ...
  if (section.type === 'certifications') {
    ...
    return <CertificationList items={items} titleClassName="font-bold text-zinc-700" issuerClassName="text-zinc-600" dateClassName="shrink-0 text-xs text-zinc-500" />;
  }
      ...
      <div className="space-y-2.5">
        {items.map((item) => (
          <div key={item.id}>
            ...
            {item.language && <span className="text-xs italic text-zinc-500">{item.language}</span>}
            {item.description && <p className="mt-0.5 text-sm text-zinc-600" dangerouslySetInnerHTML={{ __html: md(item.description) }} />}
          </div>
        ))}
      </div>
...
function buildAcademicSectionHtml(section: CanonicalResume['sections'][number], lang: string): string {
  const content = section.content as unknown as Record<string, unknown>;
  ...
  if (section.type === 'work_experience') {
    ...
    return `<div class="space-y-2.5">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(item.position)}</span>${item.company ? `<span class="text-sm text-zinc-600">, ${esc(item.company)}</span>` : ''}${item.location ? `<span class="text-sm text-zinc-500">, ${esc(item.location)}</span>` : ''}</div><span class="shrink-0 text-xs text-zinc-500">${esc(dateRange(item.startDate, item.endDate, item.current, lang))}</span></div>
      ${item.description ? `<p class="mt-0.5 text-sm text-zinc-600"><span class="font-medium text-zinc-700">${lang === 'zh' ? '职责' : 'Responsibilities'}:</span> <span>${md(item.description, { listStyle: markdownListStyle('1.25rem') })}</span></p>` : ''}
      ${item.technologies?.length ? `<p class="text-xs text-zinc-500 italic">${lang === 'zh' ? '技术栈' : 'Technologies'}: ${esc(item.technologies.join(', '))}</p>` : ''}
      ${item.highlights?.length ? `<div class="mt-0.5"><p class="mb-0.5 text-xs font-medium text-zinc-500">${lang === 'zh' ? '主要成就' : 'Key Achievements'}:</p><ul class="list-disc pl-5">${buildHighlights(item.highlights, 'text-sm text-zinc-600')}</ul></div>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if (section.type === 'education') {
    const items = (content as unknown as EducationContent).items || [];
    return `<div class="space-y-2.5">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(degreeField(item.degree, item.field))}</span></div><span class="shrink-0 text-xs text-zinc-500">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span></div>
      <p class="text-sm text-zinc-600">${esc(item.institution)}${item.location ? `, ${esc(item.location)}` : ''}</p>
      ${item.gpa ? `<p class="text-xs text-zinc-500">GPA: ${esc(item.gpa)}</p>` : ''}
      ${item.highlights?.length ? `<ul class="mt-0.5 list-disc pl-5">${buildHighlights(item.highlights, 'text-sm text-zinc-600')}</ul>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if (section.type === 'skills') {
    ...
    return `<div class="space-y-2">${categories.map((category) =>
      `<div><p class="text-sm font-bold text-zinc-700">${esc(category.name)}</p>${category.skills?.length ? `<ul class="mt-0.5 list-disc pl-4">${category.skills.map((skill) => `<li class="text-sm text-zinc-600">${esc(skill)}</li>`).join('')}</ul>` : ''}</div>`
    ).join('')}</div>`;
  }
  ...
  if (section.type === 'projects') {
    const items = (content as unknown as ProjectsContent).items || [];
    return `<div class="space-y-2.5">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(item.name)}</span>${item.url ? `<span class="text-xs text-zinc-500 ml-1">[${esc(item.url)}]</span>` : ''}</div>${item.startDate ? `<span class="shrink-0 text-xs text-zinc-500">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span>` : ''}</div>
  ...
  if (section.type === 'certifications') {
    const items = (content as unknown as CertificationsContent).items || [];
    return buildCertificationListHtml(items, { titleClass: 'font-bold text-zinc-700', issuerClass: 'text-zinc-600', dateClass: 'shrink-0 text-xs text-zinc-500' });
  }

  if (section.type === 'languages') {
    const items = (content as unknown as LanguagesContent).items || [];
    return `<p class="text-sm text-zinc-600">${items.map((item, index) => `<span><span class="font-bold text-zinc-700">${esc(item.language)}</span> (${esc(item.proficiency)})</span>${index < items.length - 1 ? '; ' : ''}`).join('')}</p>`;
  }

  if (section.type === 'github') {
    const items = (content as unknown as GitHubContent).items || [];
    return `<div class="space-y-2.5">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><span class="text-sm font-bold text-zinc-800">${esc(item.name)}${item.repoUrl ? ` <a href="${esc(item.repoUrl)}" target="_blank" rel="noopener noreferrer" class="ml-1 text-xs font-normal text-blue-500">${esc(item.repoUrl)}</a>` : ''}</span><span class="text-xs text-zinc-500">★ ${item.stars?.toLocaleString() ?? 0}</span></div>
      ${item.language ? `<span class="text-xs text-zinc-500 italic">${esc(item.language)}</span>` : ''}
      ${item.description ? `<p class="mt-0.5 text-sm text-zinc-600">${md(item.description)}</p>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if (section.type === 'custom') {
    ...
    return `<div class="space-y-2">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(item.title)}</span>${item.subtitle ? `<span class="text-sm text-zinc-600">, ${esc(item.subtitle)}</span>` : ''}</div>${item.date ? `<span class="shrink-0 text-xs text-zinc-500">${esc(item.date)}</span>` : ''}</div>
      ${item.description ? `<p class="mt-0.5 text-sm text-zinc-600">${md(item.description)}</p>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if ('items' in content && Array.isArray(content.items)) {
    const items = content.items as GenericItem[];
    return `<div class="space-y-1">${items.map((item) => `<div><span class="text-sm font-bold text-zinc-700">${esc(item.name || item.title || item.language || '')}</span>${item.description ? `<p class="text-sm text-zinc-600">${md(item.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
...
function buildContactHtml(pi: ReturnType<typeof getPersonalInfo>): string {
  ...
  const renderRow = (entries: typeof row1) =>
    entries.map((entry) => `<span style="display:inline-flex;align-items:center;gap:4px;margin:2px 6px"><span style="color:${iconColor}">${entry.htmlIcon}</span><span style="color:#52525b">${esc(entry.value)}</span></span>`).join('');

  const firstRow = row1.length > 0
    ? `<div style="margin-top:6px;font-size:12px;text-align:center">${renderRow(row1)}</div>`
    : '';
  const secondRow = row2.length > 0
    ? `<div style="margin-top:${row1.length > 0 ? '4px' : '6px'};font-size:12px;text-align:center">${renderRow(row2)}</div>`
    : '';

  return firstRow + secondRow;
}

export function buildAcademicHtml(resume: CanonicalResume): string {
  const pi = getPersonalInfo(resume);
  const sections = visibleSections(resume.sections);
  const lang = resume.language || 'en';
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:'Computer Modern','CMU Serif',Georgia,'Times New Roman',serif">
    <div class="mb-6">
      <div class="${pi.avatar ? 'flex items-center gap-4' : 'text-center'}">
        ...
        <div>
          ...
          ${pi.jobTitle ? `<p class="mt-0.5 text-base text-zinc-700 italic">${esc(pi.jobTitle)}</p>` : ''}
          ${buildContactHtml(pi)}
        </div>
      </div>
    ...
    ${sections.map((section) => `<div class="mb-4" data-section>
      <h2 class="mb-1.5 text-xs font-bold uppercase tracking-[0.25em] text-zinc-800" style="border-bottom:1px solid #d4d4d8;padding-bottom:2px">${esc(section.title)}</h2>
      ${buildAcademicSectionHtml(section, lang)}
    </div>`).join('')}
  </div>`;
}

export const academicTemplate: UnifiedTemplate = {
  id: 'academic',
  name: 'Academic',
  PreviewComponent: AcademicPreview,
  buildHtml: buildAcademicHtml,
};
