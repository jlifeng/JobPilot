...
export function CleanPreview({ resume }: TemplateProps): React.ReactElement {
    ...
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: 'Inter, sans-serif' }}>
      <div className="mb-6">
        <div className="flex items-center gap-4">
          {pi.avatar && <AvatarImage src={pi.avatar} avatarStyle={resume.themeConfig.avatarStyle} size={64} className="shrink-0" style={{ border: `2px solid ${BLUE}` }} />}
          <div>
        ...
        {contacts.length > 0 && <div className="mt-3 flex flex-wrap gap-4 text-sm text-zinc-500">{contacts.map((contact, index) => <span key={index}>{contact}</span>)}</div>}
      ...
      {sections.map((section) => (
        <div key={section.id} className="mb-5" data-section>
          ...
          <CleanSectionContent section={section} lang={lang} />
        </div>
      ))}
    </div>
  );
...
function TechPill({ children }: { children: React.ReactNode }): React.ReactElement {
  return <span className="rounded-full border px-2 py-0.5 text-[10px] font-medium" style={{ borderColor: TEAL, color: TEAL }}>{children}</span>;
}

function SkillPill({ children }: { children: React.ReactNode }): React.ReactElement {
  return <span className="rounded-full border px-3 py-0.5 text-xs font-medium" style={{ borderColor: TEAL, color: TEAL }}>{children}</span>;
}

function BulletList({ items, className = 'mt-0.5 list-disc pl-4' }: { items: string[]; className?: string }): React.ReactElement {
...
function CleanSectionContent({
      ...
      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.id}>
            ...
            {item.technologies?.length > 0 && <div className="mt-1 flex flex-wrap gap-1">{item.technologies.map((technology) => <TechPill key={technology}>{technology}</TechPill>)}</div>}
  ...
  if (section.type === 'skills') {
    ...
    return <div className="flex flex-wrap gap-2">{categories.flatMap((category) => category.skills.map((skill) => <SkillPill key={`${category.id}-${skill}`}>{skill}</SkillPill>))}</div>;
  }
  ...
  if (section.type === 'certifications') {
    ...
    return <CertificationList items={items} titleClassName="font-semibold" issuerClassName="text-zinc-600" titleStyle={{ color: BLUE }} />;
  }
  ...
  if (section.type === 'languages') {
    ...
    return (
      <div className="flex flex-wrap gap-x-6 gap-y-1">
        {items.map((item) => <span key={item.id} className="text-sm"><span className="font-semibold" style={{ color: BLUE }}>{item.language}</span><span className="text-zinc-500"> — {item.proficiency}</span></span>)}
      </div>
    );
  }
    ...
    return (
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: TEAL }}>{item.name}
                {item.repoUrl && (
...
function buildCleanSectionHtml(section: CanonicalResume['sections'][number], lang: string): string {
  const content = section.content as unknown as Record<string, unknown>;
  ...
  if (section.type === 'work_experience') {
    const items = (content as unknown as WorkExperienceContent).items || [];
    return `<div class="space-y-4">${items.map((item) => `<div><div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(item.position)}</span>${item.company ? `<span class="text-sm" style="color:${TEAL}"> | ${esc(item.company)}</span>` : ''}${item.location ? `<span class="text-sm text-zinc-400"> , ${esc(item.location)}</span>` : ''}</div><span class="shrink-0 text-xs text-zinc-400">${esc(dateRange(item.startDate, item.endDate, item.current, lang))}</span></div>${item.description ? `<p class="mt-1 text-sm text-zinc-600"><span class="font-medium text-zinc-700">${lang === 'zh' ? '职责' : 'Responsibilities'}:</span> <span>${md(item.description)}</span></p>` : ''}${item.technologies?.length ? `<div class="mt-1 flex flex-wrap gap-1">${item.technologies.map((technology) => `<span class="rounded-full border px-2 py-0.5 text-[10px] font-medium" style="border-color:${TEAL};color:${TEAL}">${esc(technology)}</span>`).join('')}</div>` : ''}${item.highlights?.length ? `<div class="mt-1"><p class="text-xs font-medium text-zinc-500">${lang === 'zh' ? '主要成就' : 'Key Achievements'}:</p><ul class="mt-0.5 list-disc pl-4">${buildHighlights(item.highlights, 'text-sm text-zinc-600')}</ul></div>` : ''}</div>`).join('')}</div>`;
  }
  if (section.type === 'education') {
    const items = (content as unknown as EducationContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div><div class="flex items-baseline justify-between"><div><span class="text-sm font-bold text-zinc-800">${esc(degreeField(item.degree, item.field))}</span>${item.institution ? `<span class="text-sm text-zinc-500"> — ${esc(item.institution)}</span>` : ''}${item.location ? `<span class="text-sm text-zinc-400"> , ${esc(item.location)}</span>` : ''}</div><span class="shrink-0 text-xs text-zinc-400">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span></div>${item.gpa ? `<p class="text-sm text-zinc-500">GPA: ${esc(item.gpa)}</p>` : ''}${item.highlights?.length ? `<ul class="mt-1 list-disc pl-4">${buildHighlights(item.highlights, 'text-sm text-zinc-600')}</ul>` : ''}</div>`).join('')}</div>`;
  }
  if (section.type === 'skills') {
    const categories = (content as unknown as SkillsContent).categories || [];
    return `<div class="flex flex-wrap gap-2">${categories.flatMap((category) => category.skills.map((skill) => `<span class="rounded-full border px-3 py-0.5 text-xs font-medium" style="border-color:${TEAL};color:${TEAL}">${esc(skill)}</span>`)).join('')}</div>`;
  }
  if (section.type === 'projects') {
    const items = (content as unknown as ProjectsContent).items || [];
    return `<div class="space-y-4">${items.map((item) => `<div><div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${TEAL}">${esc(item.name)}</span>${item.startDate ? `<span class="shrink-0 text-xs text-zinc-400">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span>` : ''}</div>${item.description ? `<p class="mt-1 text-sm text-zinc-600">${md(item.description)}</p>` : ''}${item.technologies?.length ? `<div class="mt-1 flex flex-wrap gap-1">${item.technologies.map((technology) => `<span class="rounded-full border px-2 py-0.5 text-[10px] font-medium" style="border-color:${TEAL};color:${TEAL}">${esc(technology)}</span>`).join('')}</div>` : ''}${item.highlights?.length ? `<ul class="mt-1 list-disc pl-4">${buildHighlights(item.highlights, 'text-sm text-zinc-600')}</ul>` : ''}</div>`).join('')}</div>`;
  }
  if (section.type === 'certifications') {
    const items = (content as unknown as CertificationsContent).items || [];
    return buildCertificationListHtml(items, { titleClass: 'font-semibold', issuerClass: 'text-zinc-600', titleStyle: `color:${BLUE}` });
  }
  if (section.type === 'languages') {
    const items = (content as unknown as LanguagesContent).items || [];
    return `<div class="flex flex-wrap gap-x-6 gap-y-1">${items.map((item) => `<span class="text-sm"><span class="font-semibold" style="color:${BLUE}">${esc(item.language)}</span><span class="text-zinc-500"> — ${esc(item.proficiency)}</span></span>`).join('')}</div>`;
  }
  if (section.type === 'github') {
    const items = (content as unknown as GitHubContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div><div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${TEAL}">${esc(item.name)}${item.repoUrl ? ` <a href="${esc(item.repoUrl)}" target="_blank" rel="noopener noreferrer" class="ml-1 text-xs font-normal text-blue-500">${esc(item.repoUrl)}</a>` : ''}</span><span class="text-xs text-zinc-400">★ ${item.stars?.toLocaleString() ?? 0}</span></div>${item.language ? `<span class="text-xs text-zinc-500">${esc(item.language)}</span>` : ''}${item.description ? `<p class="mt-1 text-sm text-zinc-600">${md(item.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
  if (section.type === 'custom') {
    const items = (content as unknown as CustomContent).items || [];
    return `<div class="space-y-2">${items.map((item) => `<div><div class="flex items-baseline justify-between"><div><span class="text-sm font-semibold" style="color:${BLUE}">${esc(item.title)}</span>${item.subtitle ? `<span class="text-sm text-zinc-500"> — ${esc(item.subtitle)}</span>` : ''}</div>${item.date ? `<span class="shrink-0 text-xs text-zinc-400">${esc(item.date)}</span>` : ''}</div>${item.description ? `<p class="mt-0.5 text-sm text-zinc-600">${md(item.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
...
function buildContactHtml(pi: ReturnType<typeof getPersonalInfo>): string {
  ...
  return `<div class="mt-3 flex flex-wrap gap-4 text-sm text-zinc-500">${contacts.map((contact) => `<span>${esc(contact)}</span>`).join('')}</div>`;
}

export function buildCleanHtml(resume: CanonicalResume): string {
  const pi = getPersonalInfo(resume);
  const sections = visibleSections(resume.sections);
  const lang = resume.language || 'en';
  ...
  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Inter,sans-serif">
    <div class="mb-6">
      <div class="flex items-center gap-4">
        ${pi.avatar ? `<img src="${esc(pi.avatar)}" alt="" class="h-16 w-16 shrink-0 rounded-full border-2 object-cover" style="border-color:${BLUE}"/>` : ''}
        <div>
          <h1 class="text-2xl font-bold" style="color:${BLUE}">${esc(pi.fullName || 'Your Name')}</h1>
          ${pi.jobTitle ? `<p class="mt-0.5 text-base" style="color:${TEAL}">${esc(pi.jobTitle)}</p>` : ''}
        </div>
      </div>
      ${buildContactHtml(pi)}
      <div class="mt-3 h-0.5 w-full rounded" style="background:linear-gradient(90deg,${BLUE},${TEAL})"></div>
    </div>
    ${sections.map((section) => `<div class="mb-5" data-section><h2 class="mb-2 text-sm font-bold uppercase tracking-wider" style="color:${BLUE}">${esc(section.title)}</h2>${buildCleanSectionHtml(section, lang)}</div>`).join('')}
  </div>`;
}

export const cleanTemplate: UnifiedTemplate = {
  id: 'clean',
  name: 'Clean',
