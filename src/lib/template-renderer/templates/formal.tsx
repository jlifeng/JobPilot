...
export function FormalPreview({ resume }: TemplateProps): React.ReactElement {
    ...
    <div className="mx-auto max-w-[210mm] bg-white shadow-lg" style={{ fontFamily: FONT_FAMILY }}>
      ...
      {sections.map((section) => (
        <div key={section.id} className="mb-5" data-section>
          ...
          <FormalSectionContent section={section} lang={lang} />
        </div>
      ))}
    </div>
  );
...
function FormalSectionContent({
      ...
      <div className="space-y-1">
        {categories.map((category) => (
          <div key={category.id} className="flex text-sm">
            <span className="w-32 shrink-0 font-semibold" style={{ color: DARK_GREEN }}>{category.name}:</span>
  ...
  if (section.type === 'certifications') {
    ...
    return <CertificationList items={items} titleClassName="font-bold" issuerClassName="text-zinc-600" dateClassName="shrink-0 text-xs italic text-zinc-400" titleStyle={{ color: DARK_GREEN }} />;
  }
    ...
    return (
      <div className="space-y-3">
        {items.map((item) => (
          <div key={item.id}>
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color: DARK_GREEN }}>{item.name}
                {item.repoUrl && (
...
function buildFormalSectionHtml(section: CanonicalResume['sections'][number], lang: string): string {
  const content = section.content as unknown as Record<string, unknown>;
  ...
  if (section.type === 'work_experience') {
    ...
    return `<div class="space-y-4">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.position)}</span>${item.company ? `<span class="text-sm text-zinc-600">, ${esc(item.company)}</span>` : ''}${item.location ? `<span class="text-sm text-zinc-400"> (${esc(item.location)})</span>` : ''}</div><span class="shrink-0 text-xs italic text-zinc-400">${esc(dateRange(item.startDate, item.endDate, item.current, lang))}</span></div>
  ...
  if (section.type === 'education') {
    ...
    return `<div class="space-y-3">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.institution)}</span>${item.location ? `<span class="text-sm text-zinc-400"> (${esc(item.location)})</span>` : ''}</div><span class="shrink-0 text-xs italic text-zinc-400">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span></div>
  ...
  if (section.type === 'skills') {
    const categories = (content as unknown as SkillsContent).categories || [];
    return `<div class="space-y-1">${categories.map((category) =>
      `<div class="flex text-sm"><span class="w-32 shrink-0 font-semibold" style="color:${DARK_GREEN}">${esc(category.name)}:</span><span class="text-zinc-600">${esc(category.skills.join(', '))}</span></div>`
    ).join('')}</div>`;
  }
  ...
  if (section.type === 'projects') {
    const items = (content as unknown as ProjectsContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.name)}</span>${item.startDate ? `<span class="shrink-0 text-xs italic text-zinc-400">${esc(dateRange(item.startDate, item.endDate, true, lang))}</span>` : ''}</div>
  ...
  if (section.type === 'certifications') {
    const items = (content as unknown as CertificationsContent).items || [];
    return buildCertificationListHtml(items, { titleClass: 'font-bold', issuerClass: 'text-zinc-600', dateClass: 'shrink-0 text-xs italic text-zinc-400', titleStyle: `color:${DARK_GREEN}` });
  }

  if (section.type === 'languages') {
    const items = (content as unknown as LanguagesContent).items || [];
    return `<div class="space-y-1">${items.map((item) =>
      `<div class="flex text-sm"><span class="w-32 shrink-0 font-semibold" style="color:${DARK_GREEN}">${esc(item.language)}:</span><span class="text-zinc-600">${esc(item.proficiency)}</span></div>`
    ).join('')}</div>`;
  }
  ...
  if (section.type === 'github') {
    const items = (content as unknown as GitHubContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.name)}${item.repoUrl ? ` <a href="${esc(item.repoUrl)}" target="_blank" rel="noopener noreferrer" class="ml-1 text-xs font-normal text-blue-500">${esc(item.repoUrl)}</a>` : ''}</span><span class="shrink-0 text-xs italic text-zinc-400">★ ${item.stars?.toLocaleString() ?? 0}</span></div>
  ...
  if (section.type === 'custom') {
    const items = (content as unknown as CustomContent).items || [];
    return `<div class="space-y-3">${items.map((item) => `<div>
      <div class="flex items-baseline justify-between"><div><span class="text-sm font-bold" style="color:${DARK_GREEN}">${esc(item.title)}</span>${item.subtitle ? `<span class="text-sm text-zinc-500"> — ${esc(item.subtitle)}</span>` : ''}</div>${item.date ? `<span class="shrink-0 text-xs italic text-zinc-400">${esc(item.date)}</span>` : ''}</div>
      ${item.description ? `<p class="mt-0.5 text-sm text-zinc-600">${md(item.description)}</p>` : ''}
    </div>`).join('')}</div>`;
  }
  ...
  if ('items' in content && Array.isArray(content.items)) {
    const items = content.items as GenericItem[];
    return `<div class="space-y-2">${items.map((item) => `<div><span class="text-sm font-medium" style="color:${DARK_GREEN}">${esc(item.name || item.title || item.language || '')}</span>${item.description ? `<p class="text-sm text-zinc-600">${md(item.description)}</p>` : ''}</div>`).join('')}</div>`;
  }
...
export function buildFormalHtml(resume: CanonicalResume): string {
  const pi = getPersonalInfo(resume);
  const sections = visibleSections(resume.sections);
  const lang = resume.language || 'en';

  return `<div class="mx-auto max-w-[210mm] bg-white shadow-lg" style="font-family:Georgia,'Times New Roman',serif">
    <div class="mb-6 border-b-2 pb-4" style="border-color:${DARK_GREEN}">
      <div class="flex items-center justify-center gap-4">
        ${pi.avatar ? `<img src="${esc(pi.avatar)}" alt="" class="h-16 w-16 shrink-0 rounded-full border-2 object-cover" style="border-color:${DARK_GREEN}"/>` : ''}
        <div class="text-center">
          <h1 class="text-2xl font-bold" style="color:${DARK_GREEN}">${esc(pi.fullName || 'Your Name')}</h1>
          ${pi.jobTitle ? `<p class="mt-0.5 text-base text-zinc-500">${esc(pi.jobTitle)}</p>` : ''}
        </div>
      </div>
      ${buildContactHtml(pi)}
    </div>
    ${sections.map((section) => `<div class="mb-5" data-section>
      <div class="mb-2 flex items-center gap-2">
        <h2 class="shrink-0 text-sm font-bold uppercase tracking-wider" style="color:${DARK_GREEN}">${esc(section.title)}</h2>
        <div class="h-px flex-1 bg-zinc-200"></div>
      </div>
      ${buildFormalSectionHtml(section, lang)}
    </div>`).join('')}
  </div>`;
}

export const formalTemplate: UnifiedTemplate = {
  id: 'formal',
  name: 'Formal',
  PreviewComponent: FormalPreview,
  buildHtml: buildFormalHtml,
};
