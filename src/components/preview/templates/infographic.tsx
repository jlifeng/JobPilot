      ...
      <div className="relative overflow-hidden px-8 py-8" style={{ background: 'linear-gradient(135deg, #1e40af, #7c3aed)' }}>
        <div className="relative flex items-center gap-5">
          ...
          <div>
            ...
            <ContactInfo pi={pi} iconColor="rgba(255,255,255,0.5)" style={{ color: 'rgba(255,255,255,0.7)' }} align="left" />
          </div>
        </div>
      </div>
    ...
    return (
      <div className="space-y-3">
        {items.map((item: any) => (
          <div key={item.id} className="rounded-lg border border-zinc-100 p-4">
            <div className="flex items-baseline justify-between">
              <span className="text-sm font-bold" style={{ color }}>{item.name}{item.repoUrl && (<a href={item.repoUrl} target="_blank" rel="noopener noreferrer" className="ml-1 text-xs font-normal text-blue-500 hover:underline">{item.repoUrl}</a>)}</span>
